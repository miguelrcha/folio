...
export function EditProjectsModal({ profileId }: { profileId: string }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      // Pulls new/updated repos from GitHub before listing, so someone who
      // just created a repo doesn't have to wait for another manual sync.
  ...
  return (
    <>
      <button
        onClick={handleOpen}
        aria-label={t("modal.projects.ariaEdit")}
